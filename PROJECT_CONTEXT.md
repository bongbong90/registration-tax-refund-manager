...
- `case_create_dialog.py`
  - 납부일 인라인 달력 토글 구조를 `cal_container` 기반으로 재정리
  - 달력 열림 후 닫힘 시 다이얼로그 높이 복원 처리로 납부일-세액 사이 공백 제거
  - `setup_combo()` 적용으로 환급사유/거래처 드롭다운 검은 배경/외곽선 제거
  - 콤보박스 우측 화살표를 `▾` 오버레이로 표시해 깨짐(`=` 형태) 방지
- `theme.py`
  - 전역 `QComboBox::down-arrow` 커스텀 삼각형 제거
  - 드롭다운 영역을 연한 분리선/배경으로 통일해 기본 렌더링 충돌 최소화
- `case_detail_dialog.py`
  - 기본정보/진행이력/메모 탭 라이트 테마 통일
  - 진행이력 테이블 헤더/행 스타일 라이트 톤 정리 및 처리자 툴팁 유지
  - 처리일시/내용/처리자 컬럼 폭을 140 / stretch / 150으로 조정
  - 상세 탭 콤보박스에도 `▾` 화살표 오버레이 적용
- `case_table.py`
  - 메인 상태 필터 콤보박스 화살표 렌더링 보정(`▾` 오버레이)
- 실행 검증 및 캡처
  - `python main.py` 실행 확인
  - 산출 이미지: `ui_p15_5_dialog.png`, `ui_p15_5_calendar.png`, `ui_p15_5_combo.png`,
    `ui_p15_5_detail.png`, `ui_p15_5_history.png`
